...
def classification_metrics(y_true: list, y_pred: list) -> dict:
    ...
    evals = pd.DataFrame(classification_report(y_true, y_pred, labels=[0, 1], output_dict=True))
    f1_class0,f1_class1,accuracy,macro,weight = evals.loc["f1-score"]
    precision_class0,precision_class1,accuracy,macro,weight = evals.loc["precision"]
    recall_class0,recall_class1,accuracy,macro,weight = evals.loc["recall"]
